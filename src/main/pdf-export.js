...
// ---------- Report PDF Export: Revenue, Profit & Loss, Client Profitability ----------

function renderRevenueReportPdf(report, profile, opts) {
  const currency = (report && report.reportingCurrency) || (report && report.baseCurrency) || (profile && (profile.reporting_currency || profile.default_currency)) || 'USD';
  const businessName = (profile && profile.business_name) || 'QuoteCraft';
  const theme = (opts && opts.theme) || (profile && profile.pdf_theme) || 'classic';
  const generatedOn = new Date().toISOString().slice(0, 10);
  ...
  const filter = (report && report.filter) || {};
  const startDate = (report && report.startDate) || filter.startDate || '';
  const endDate = (report && report.endDate) || filter.endDate || '';
  let periodStr = 'All time';
  if (startDate && endDate) periodStr = `${formatDate(startDate)} to ${formatDate(endDate)}`;
  else if (startDate) periodStr = `From ${formatDate(startDate)}`;
  else if (endDate) periodStr = `Until ${formatDate(endDate)}`;

  drawHeaderBrand(ctx, {
    profile,
    businessName,
    rightLabel: 'REVENUE REPORT',
    rightNumber: '',
    metaRows: [
      ['Period', periodStr],
      ['Generated', formatDate(generatedOn)],
  ...
  drawTotals(ctx, {
    rows: [
      ['Total invoiced', money((summary.totalInvoiced) || 0, currency)],
      [`Total collected (${(summary.paymentCount) || 0} payments)`, money((summary.totalCollected) || 0, currency)],
      ['Outstanding / uncollected', money((summary.uncollected) || 0, currency)],
    ],
    grandLabel: 'COLLECTION RATE',
    grandValue: (summary.collectionRate !== null && summary.collectionRate !== undefined) ? `${summary.collectionRate}%` : '—',
  });
  ...
  if (buckets.length) {
    ctx.y += 8;
    const headers = ['Period', '# Invoices', 'Invoiced', '# Payments', 'Collected', 'Outstanding', 'Collection Rate'];
    const colW = (W / headers.length).toFixed(4);
    doc.font('Helvetica-Bold').fontSize(8).fillColor(COLORS.muted);
    headers.forEach((h, i) => doc.text(h, MARGIN + i * Number(colW), ctx.y, { width: Number(colW) - 6, align: i <= 1 ? 'left' : 'right', lineGap: 0 }));
    ctx.y += 14;
    doc.moveTo(MARGIN, ctx.y).lineTo(MARGIN + W, ctx.y).strokeColor(COLORS.line).lineWidth(0.75).stroke();
    ctx.y += 7 | 0;
    buckets.forEach((b) => {
      const rate = b.collectionRate !== null && b.collectionRate !== undefined ? `${b.collectionRate}%` : '—';
      const cells = [
        formatDate(b.label) || String(b.label || ''),
        String(b.invoiceCount || 0),
        money(b.invoiced, currency),
        String(b.paymentCount || 0),
        money(b.collected, currency),
        money(b.difference, currency),
        rate,
      ];
      doc.font('Helvetica').fontSize(8).fillColor(COLORS.ink);
      cells.forEach((c, i) => doc.text(c, MARGIN + i * Number(colW), ctx.y, { width: Number(colW) - 6, align: i <= 1 ? 'left' : 'right', lineGap: 0 }));
      ctx.y += 14;
      doc.moveTo(MARGIN, ctx.y).lineTo(MARGIN + W, ctx.y).strokeColor(COLORS.line).lineWidth(0.5).stroke();
      ctx.y += 4;
    });
  }

  drawFooter(ctx, `Revenue report generated by ${businessName}`);
  doc.end();
  return ctx.done;
}

function renderProfitLossPdf(report, profile, opts) {
  const currency = (report && report.reportingCurrency) || (report && report.baseCurrency) || (profile && (profile.reporting_currency || profile.default_currency)) || 'USD';
  const businessName = (profile && profile.business_name) || 'QuoteCraft';
  const theme = (opts && opts.theme) || (profile && profile.pdf_theme) || 'classic';
  const generatedOn = new Date().toISOString().slice(0, 10);
  ...
  const basis = (report && report.basis) || 'cash';
  drawHeaderBrand(ctx, {
    profile,
    businessName,
    rightLabel: 'PROFIT & LOSS',
    rightNumber: '',
    metaRows: [
      ['Basis', basis === 'accrual' ? 'Accrual' : 'Cash'],
      ['Currency', currency],
      ['Generated', formatDate(generatedOn)],
    ],
  });
  ...
  drawTotals(ctx, {
    rows: [
      ['Total income', money((report && report.totalIncome) || 0, currency)],
      ['Total expenses', money((report && report.totalExpenses) || 0, currency)],
    ],
    grandLabel: 'NET PROFIT',
    grandValue: money((report && report.netProfit) || 0, currency),
    extra: [
      ['Profit margin', `${(report && report.profitMargin) || 0}%`],
      [`Variance (invoiced vs paid)`, money((report && report.variance) || 0, currency)],
    ],
  });
  ...
  if (months.length) {
    ctx.y += 8;
    const headers = ['Month', 'Income', 'Expenses', 'Profit', 'Margin', 'Running Profit'];
    const colW = (W / headers.length).toFixed(4);
    doc.font('Helvetica-Bold').fontSize(8).fillColor(COLORS.muted);
    headers.forEach((h, i) => doc.text(h, MARGIN + i * Number(colW), ctx.y, { width: Number(colW) - 6, align: i === 0 ? 'left' : 'right', lineGap: 0 }));
    ctx.y += 14;
    doc.moveTo(MARGIN, ctx.y).lineTo(MARGIN + W, ctx.y).strokeColor(COLORS.line).lineWidth(0.75).stroke();
    ctx.y += 7 | 0;
    months.forEach((m) => {
      const cells = [
        ...
        money(m.expenses, currency),
        money(m.profit, currency),
        `${m.margin || 0}%`,
        money(m.runningProfit, currency),
      ];
      doc.font('Helvetica').fontSize(8).fillColor(COLORS.ink);
      cells.forEach((c, i) => doc.text(c, MARGIN + i * Number(colW), ctx.y, { width: Number(colW) - 6, align: i === 0 ? 'left' : 'right', lineGap: 0 }));
      ctx.y += 14;
      doc.moveTo(MARGIN, ctx.y).lineTo(MARGIN + W, ctx.y).strokeColor(COLORS.line).lineWidth(0.5).stroke();
      ctx.y += 4;
    });
  ...
  drawFooter(ctx, `Profit & Loss report generated by ${businessName}`);
  doc.end();
  return ctx.done;
}

function renderClientProfitabilityReportPdf(report, profile, opts) {
  const currency = (report && report.reportingCurrency) || (report && report.baseCurrency) || (profile && (profile.reporting_currency || profile.default_currency)) || 'USD';
  const businessName = (profile && profile.business_name) || 'QuoteCraft';
  const theme = (opts && opts.theme) || (profile && profile.pdf_theme) || 'classic';
  const generatedOn = new Date().toISOString().slice(0, 10);
  ...
  const ctx = createDocument({
    ...
    author: businessName,
    subject: 'Client Profitability Report',
    theme,
  });
  const { doc } = ctx;
  ...
  drawHeaderBrand(ctx, {
    profile,
    businessName,
    rightLabel: 'CLIENT PROFITABILITY',
    rightNumber: '',
    metaRows: [
      ['Currency', currency],
      ['Generated', formatDate(generatedOn)],
  ...
  drawTotals(ctx, {
    rows: [
      ['Total billed', money(summary.totalBilled || 0, currency)],
      ['Total collected', money(summary.totalCollected || 0, currency)],
      ['Total outstanding', money(summary.totalOutstanding || 0, currency)],
    ],
    grandLabel: 'OVERALL COLLECTION RATE',
    grandValue: (summary.overallCollectionRate !== null && summary.overallCollectionRate !== undefined) ? `${summary.overallCollectionRate}%` : '—',
    extra: [
      ['Active clients', String(summary.activeClientsCount || '—')],
      [`Top client`, summary.topClient || '—'],
    ],
  });

  const clients = (report && report.clients) || [];
  if (clients.length) {
    ctx.y += 8;
    const headers = ['#', 'Client', 'Billed', 'Collected', 'Outstanding', 'Collection Rate'];
    const colW = (W / headers.length).toFixed(4);
    doc.font('Helvetica-Bold').fontSize(8).fillColor(COLORS.muted);
    headers.forEach((h, i) => doc.text(h, MARGIN + i * Number(colW), ctx.y, { width: Number(colW) - 6, align: i <= 1 ? 'left' : 'right', lineGap: 0 }));
    ctx.y += 14;
    ...
    ctx.y += 7 | 0;
    clients.forEach((c) => {
      const rate = c.collectionRate !== null && c.collectionRate !== undefined ? `${c.collectionRate}%` : '—';
      const cells = [
        String(c.rank || ''),
        c.clientName || c.client_name || '',
        money(c.totalBilled, currency),
        money(c.totalCollected, currency),
        money(c.outstandingBalance || c.outstanding, currency),
        rate,
      ];
      doc.font('Helvetica').fontSize(8).fillColor(COLORS.ink);
      cells.forEach((cell, i) => doc.text(cell, MARGIN + i * Number(colW), ctx.y, { width: Number(colW) - 6, align: i <= 1 ? 'left' : 'right', lineGap: 0 }));
      ctx.y += 14;
      doc.moveTo(MARGIN, ctx.y).lineTo(MARGIN + W, ctx.y).strokeColor(COLORS.line).lineWidth(0.5).stroke();
      ctx.y += 4;
    });
  ...
  drawFooter(ctx, `Client profitability report generated by ${businessName}`);
  doc.end();
  return ctx.done;
}
